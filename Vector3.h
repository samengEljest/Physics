...
public:
	...
	Vector3 operator-(Vector3 tmp); ///< owerloading - operator to work with Vector-Vector
	Vector3 operator*(float tmp); ///< owerloading * operator to work with Vector*float
	...
	double dot(Vector3 tmp1, Vector3 tmp2);
	Vector3 cross(Vector3 tmp); ///< used for the cross product between two 3d vectors
	Vector3 cross(Vector3 tmp1, Vector3 tmp2);
	...
	void addScaledVector(Vector3 &tmp, float scale);
	float scalarProduct(Vector3 &vectorIn);
	float magnitude(); // POP POP!
};